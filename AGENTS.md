...
- `network.js` wraps PeerJS (via CDN) and manages room codes, connection lifecycle, and the host/guest roles. It attaches a `peer.on('disconnected')` handler on every peer that calls `peer.reconnect()` automatically when the signaling WebSocket drops (e.g. mobile backgrounding). `restoreHost(roomCode, cbs)` and `restoreGuest(roomCode, cbs)` recreate the full peer from scratch with the same room code, used by `game.js`'s reconnect state machine. `getLastRoomCode()` exposes the saved code. Key internal invariants: (1) `setupConnection(connRef)` and `startOpenPoll(connRef)` take an explicit `DataConnection` reference rather than reading the module-level `conn`, preventing stale references when `conn` is replaced during reconnects; (2) all paths that call `markConnected()` (in both `setupConnection` and `startOpenPoll`) guard with `connRef === conn` before firing, so a stale connection racing against a newer one cannot flip `connected=true` or fire `onConnected` for the wrong connection; `startOpenPoll` also cancels itself immediately when `connRef !== conn`; (3) `close`/`error` handlers guard with `connRef === conn` before firing callbacks, so events from replaced connections are silently dropped; (4) the `intentionalDestroy` flag suppresses `onPeerClosed` when `peer.destroy()` is called deliberately (in `restoreHost`, `restoreGuest`, and `disconnect`); additionally `attachPeerLifecycleHandlers` guards the `close` handler with `peerObj !== peer` to drop late close events from replaced peer instances, preventing them from firing into the new callbacks; (5) `disconnect()` resets `isHost` and `lastRoomCode` to avoid leaking stale state into the next session; (6) `PEER_DEBUG` is `2` on `localhost` and `0` everywhere else.
